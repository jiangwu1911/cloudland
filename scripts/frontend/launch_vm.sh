...
[ $# -lt 4 ] && echo "$0 <user> <image> <vlan> <mac> [name] [ip] [cpu] [memory (m)] [hyper]" && exit -1
...
num=`sqlite3 $db_file "select count(*) from network where vlan='$vlan' and (owner='$owner' or shared='true')"`
...
if [ "$router" -ge 0 ]; then
    ...
    /opt/cloudland/bin/sendmsg "inter $hyper_id" "/opt/cloudland/scripts/backend/`basename $0` $vm_ID $img_name $vlan $vm_mac $vm_name $vm_ip $vm_cpu $vm_mem"
fi
echo "Launching vm $vm_ID!"
