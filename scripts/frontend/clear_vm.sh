...
owner=$1
vm_ID=$2
sql_ret=`sqlite3 $db_file "select vlan, ip_addr, mac_addr, hyper_name, owner from instance where inst_id='$vm_ID' and owner='$owner'"`
...
for i in $ids; do
    ...
    rt=`sqlite3 $db_file "select id from compute where hyper_name=(select router from network where vlan='$vl')"`
    /opt/cloudland/bin/sendmsg "inter $rt" "/opt/cloudland/scripts/backend/del_host.sh $vl $mac $ip"
    sqlite3 $db_file "update address set allocated='false' where IP='$vm_ip'"
done
