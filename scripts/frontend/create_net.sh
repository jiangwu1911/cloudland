...
[ $# -lt 4 ] && echo "$0 <user> <vlan> <network> <netmask> [gateway] [start_ip] [end_ip] [shared] [hyper]" && exit -1
...
[ "$vlan" -ge 4095 ] || die "Vlan number must be >= 4095"
...
num=`sqlite3 $db_file "select count(*) from network where owner='$owner'"`
...
sqlite3 $db_file "insert into network (vlan, network, netmask, gateway, start_address, end_address, owner, shared) values ($vlan, '$network', '$netmask', '$gateway', '$start_ip', '$end_ip', '$owner', '$shared')" 
if [ "$has_gw" == 'true' ]; then
...
/opt/cloudland/bin/sendmsg "inter $hyper_id" "/opt/cloudland/scripts/backend/`basename $0` $vlan $network $netmask $gateway $start_ip $end_ip"
echo "Vlan $vlan was created!"
